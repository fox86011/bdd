...
SELECT * 
FROM productos 
WHERE stock = 10 AND precio < CAST(10 AS money);
SELECT nombre, stock 
FROM productos 
...
FROM cuentas 
WHERE saldo > CAST(100 AS money) AND saldo < CAST(1000 AS money);
SELECT * 
FROM cuentas 
WHERE fecha_creacion BETWEEN CURRENT_DATE - INTERVAL '1 year' AND CURRENT_DATE;
SELECT * 
FROM cuentas 
WHERE saldo = CAST(0 AS money) OR cedula_propietario LIKE '%2';
--Script 3
...
WHERE EXTRACT(MONTH FROM fecha) = 8 
  ...
  AND EXTRACT(HOUR FROM hora) BETWEEN 8 AND 12;
SELECT * 
FROM registros_entrada 
WHERE (EXTRACT(MONTH FROM fecha) = 8 AND cedula_empleado LIKE '17%' AND EXTRACT(HOUR FROM hora) BETWEEN 8 AND 12)
   OR (EXTRACT(MONTH FROM fecha) = 9 AND cedula_empleado LIKE '08%' AND EXTRACT(HOUR FROM hora) BETWEEN 9 AND 13);
--Script 5
...
SELECT * 
FROM transacciones 
WHERE tipo = 'C' AND numero_cuenta BETWEEN CAST(222001 AS character) AND CAST(22004 AS character);
SELECT * 
FROM transacciones 
WHERE tipo = 'D' AND fecha = '2023-05-25' AND numero_cuenta BETWEEN 22007 AND 22010;
SELECT * 
FROM transacciones 
WHERE codigo BETWEEN 1 AND 5 AND numero_cuenta IN (CAST(22002 AS character), CAST(22004 AS character), CAST(22005 AS character));